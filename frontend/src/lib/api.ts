...
export const authApi = {
  ...
  updateEmail: async (data: EmailUpdateRequest): Promise<User> => {
    const response = await apiClient.patch<User>('/auth/me/email', data);
    return response.data;
  },
...
export const videosApi = {
  ...
  getVideos: async (): Promise<Video[]> => {
    ...
    if (!Array.isArray(response.data)) {
      throw new Error('Invalid videos response from API');
    }
    return response.data;
  },
...
export const processingApi = {
  ...
  getDiagnostics: async (videoId: number): Promise<ProcessingDiagnosticsResponse> => {
    const response = await apiClient.get<ProcessingDiagnosticsResponse>(`/processing/${videoId}/diagnostics`);
    return response.data;
  },
